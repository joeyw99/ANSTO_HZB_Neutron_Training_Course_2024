...
echo "ecutwfc,total_energy,time" > data.csv

for output_dir in ecut*
do
    var=$(echo "$output_dir" | cut -f 2 -d "_" | grep -Eo '[+-]?[0-9]+([.][0-9]+)?')
    ...
    hours="${hours:-0}"
    echo "$var,$value1,$(echo $hours*3600+$minutes*60+$seconds | bc)" >> data.csv
done
